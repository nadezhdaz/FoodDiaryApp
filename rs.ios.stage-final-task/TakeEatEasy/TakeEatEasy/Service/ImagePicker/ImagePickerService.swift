...
protocol ImagePickerDelegate: class {
    ...
    func cancelButtonDidClick(on imageView: ImagePickerService)
}
...
extension ImagePickerService {
    ...
    private func showSimulatorAlert(completion: ((Bool) -> Void)?) {
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            let alertVC = UIAlertController(title: "Access to the camera is unavailable from Simulator",
                                            message: "Please add photo from Simulator Photo library",
                                            preferredStyle: .alert)
            alertVC.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: {  _ in completion?(false) }))
            UIApplication.shared.windows.filter { $0.isKeyWindow }.first?
                .rootViewController?.present(alertVC, animated: true, completion: nil)
